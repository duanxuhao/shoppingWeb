<template>
  <div>
                ...
                <li v-for="item in carList">
                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a href="javascript:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.productChecked=='1'}" @click="editCart('checked',item)">
              ...
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascipt:;" @click = "toggleCheckAll">
                    <span class="checkbox-btn item-check-btn" v-bind:class="{'checked':checkedAllFlag}">
                      <svg class="icon icon-ok">
                        <use xlink:href="#icon-ok"></use>
                      </svg>
                    </span>
      ...
      <div class="md-overlay" v-show="modelConfirm" @click="closeModel"></div>
      <model v-bind:mdShow="modelConfirm" v-on:close="closeModel">
        <p slot ="message">
            确认删除此物品？
        </p>
        <div slot ="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="delCart">确认</a>
          <a class="btn btn--m" href="javascript:;"  @click="modelConfirm=false">取消</a>
        </div>
      </model>
    ...
    export default {
        ...
        computed:{
          ...
          totalPrice(){
            ...
            this.carList.forEach((item)=>{
              if(item.productChecked=="1"){
                money += parseFloat(item.productNum)*parseInt(item.productPrice);
              }
            })
            return money;
          }
        ...
        methods:{
            ...
            deleteProduct(productId){
              ...
              axios.get("/users/deleteProduct").then((response)=>{
                    let res = response.data;
            ...
            editCart(flag,item){
              if(flag=="add"){
                ...
                if(item.productNum<=1){
                 ...
                 if(item.productChecked=="1"){
                    item.productChecked="0";
                  }else{
                    item.productChecked="1";
                  }
              }
              axios.post("/users/cartEdit",{productId:item.productId,productNum:item.productNum,productChecked:item.productChecked}).then((response)=>{
                let res = response.data;
              })
            },
            toggleCheckAll(){
